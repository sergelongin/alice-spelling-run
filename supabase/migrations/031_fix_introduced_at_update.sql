-- Migration: Fix introduced_at missing from UPDATE path in push_changes
--
-- Bug: The UPDATE block for word_progress (lines 111-133 in migration 030) was missing
-- the introduced_at field, causing words to remain as "Coming Soon" on other devices
-- even after being introduced on the original device.
--
-- This migration replaces the push_changes function with the corrected version.
...
BEGIN
  ...
  -- Process updated word_progress records
  -- FIX: Added introduced_at handling (was missing in migration 030)
  FOR wp_item IN SELECT * FROM json_array_elements(
  ...
  LOOP
    ...
    UPDATE child_word_progress SET
      ...
      next_review_at = CASE
        ...
        ELSE next_review_at
      END,
      -- FIX: Added introduced_at handling
      -- Only set if currently NULL and client has a value (never overwrite existing)
      introduced_at = CASE
        WHEN introduced_at IS NULL AND wp_item->>'introduced_at' IS NOT NULL
        THEN (wp_item->>'introduced_at')::timestamptz
        ELSE introduced_at
      END,
      is_active = CASE
...
'Event-sourced push: accepts events (game_sessions, word_attempts, calibration) and metadata (word_progress, learning_progress, grade_progress). Statistics and word mastery are computed server-side. Fixed in 031: introduced_at now properly handled in UPDATE path.';
